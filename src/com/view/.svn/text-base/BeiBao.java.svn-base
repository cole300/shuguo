...
public class BeiBao extends View{
	...
	ElementGroup eleGroup;//view群组
	BtnGroup btnGroup;//button群组
	Bitmap bmp_btnGroup;//button群组背景
	
	Bitmap bmp_bg;//背景图
	
	public BeiBao(Context context, ShuguoActivity activity)
	{
		super(context);
	...
	protected void onDraw(Canvas canvas)
	{
		...
		canvas.drawText("背包", sx + 58, sy + 46, paint);
		/*button群组背景*/
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		case MotionEvent.ACTION_UP:
			...
			for(int i = 0 ; i < this.eleGroup.getGroup().size(); i ++)
			{
				...
				{
					/*打开商店交易界面*/
					BeiBaoUse bbu = new BeiBaoUse(activity.getApplicationContext(), activity);
					activity.group.addView(bbu);
					break;
				}
			}
